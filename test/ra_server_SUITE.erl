    ...
    {leader, #{cluster := #{N2 := #{next_index := 4,
     ...
     [{next_event, info, pipeline_rpcs},
      {next_event, {command, {'$ra_join', _,
        #{id := N2, voter_status := voter}, noreply}} = RaJoin}
...
leader_received_append_entries_reply_and_promotes_voter(_config) ->
    ...
    % Permanent voter
    State1 = set_peer_voter_status(State, N3, voter),
    {leader, _,
     [{next_event,info,pipeline_rpcs}]
    ...
    {leader, _,
     [{next_event,info,pipeline_rpcs},
      {next_event, {command, {'$ra_join', _, #{id := N3, voter_status := voter}, _}}}]
